...
# train an ensemble of replicas of a given model for one epoch, using the Replicated SGD algorithm.
# return the average loss, the average l2 norm of the weights, and the average l2 distance of the 
# replicas from the baricenter.
...
# handle the training loop for Replicated SGD, including logging to wandb, early stopping, estimation
# of flatness through local energy and denoising capabilities. Most of the complexity is due to 
# logging to wandb, which is done at every epoch and is quite heavy due to flatness and denoising.
...
def train_ae(
    ...
    # Note: before 18/03, l2_norm was normalized by total_params, not by its root.
    return average_error.cpu(), average_l2.cpu()


# train AE calling repeatedly train_ae. Handles early stopping and logging to wandb.
# Saves RE and l2 norm as returned by train_ae after each epoch. Similarly to train_replicated,
# most of the complexity is due to logging to wandb.
